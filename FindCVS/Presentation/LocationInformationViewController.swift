...
class LocationInformationViewController: UIViewController {
  ...
  private func bind(_ viewModel: LocationInformationViewModel) {
    ...
    viewModel.scrollToSelectedLoaction
      .emit(to: self.rx.showSelectedLoaction)
      .disposed(by: disposeBag)
    
    detailList.rx.itemSelected
      .map { $0.row }
      .bind(to: viewModel.detilListItemSelected)
      .disposed(by: disposeBag)
...
extension LocationInformationViewController: MTMapViewDelegate {
  ...
  func mapView(_ mapView: MTMapView!,
               ...
               withAccuracy accuracy: MTMapLocationAccuracy) {
    #if DEBUG
    viewModel.currentLoaction.accept(MTMapPoint(geoCoord: MTMapPointGeo(latitude: 37.394225, longitude: 127.110341)))
    #else
      viewModel.currentLocation.acccpt(location)
    #endif
  }
...
extension Reactive where Base: LocationInformationViewController {
  var presentAlert: Binder<String> {
    return Binder(base) { base, message in
      ...
      base.present(alertViewController, animated: true)
    }
  }
  
  var showSelectedLoaction: Binder<Int> {
